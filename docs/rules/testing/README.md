...
- [ ] **R010** `test/audits/` holds checks whose subject is the whole tree rather than one module — for example, a scan confirming every file under `src/sim/**`, `src/cutscene/**`, and `scripts/lib/**` has a co-located test, or a coverage-threshold assertion independent of the `vitest` reporter's own gate. It never holds a spec whose subject is a single scene or a single replay — those belong in `e2e-testing.md`, `visual-testing.md`, or `replay-testing.md`.
