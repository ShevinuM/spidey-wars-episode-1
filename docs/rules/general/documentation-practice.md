...
- [ ] **R010** A `## Sources` URL is citable only once it has been fetched and its content actually seen to contain the claim it supports — a plausible-looking path is not a citation. This phase shipped exactly one fabricated URL (a 404 disguised as a real GitHub path) that survived two reviews before a direct fetch caught it; treat an un-fetched `## Sources` entry as an open defect, not a placeholder to fill in "later."
