...
- [ ] **R001** `pnpm check` runs six gates in sequence, each proving something different — a later gate does not re-check what an earlier one already covers:
  ...
  - `pnpm lint` (`oxlint --deny-warnings`) — fast syntactic rules plus the purity layer's edit-time half (`architecture.md` R003). `.oxlintrc.json` ignores `reference/**` (vendored design source, not project code).
...
- [ ] **R005** The canary discipline in R004 binds a verification command itself, not only the gate it checks. A check that has never been observed to fail on a known-bad input is not yet a check, whatever it claims to assert. This phase found three of its own verification commands broken this way while the thing each checked was fine: an index-numbering regex that excluded digits and so could never match a 17th file, a link-checking regex that silently skipped every `../../` cross-reference form, and a `## Sources` URL nobody had actually fetched. None of the three was caught by running it — each was caught by feeding it a case it should have failed on and watching it not.

## Gates are phase-level, not per-commit

- [ ] **R006** A gate failing on one commit inside a multi-commit phase is not itself a defect. `prettier --check .` is tree-scoped: it checks every file in the working tree, not only the files a given commit touched, so a commit may legitimately land with `format` red when the file that needs reformatting belongs to a later commit in the same phase. Do not write, or read this file as implying, a rule that every individual commit must leave every gate green — the phase as a whole must, by the time it's declared done.

## Git hooks

- [ ] **R007** A lefthook (or equivalent) hook scoped by `glob` is skipped entirely when no staged file matches that glob — even when the hook's `run` template contains no `{staged_files}` placeholder to filter. A commit touching only `docs/**` does not run a hook globbed to `*.ts`/`*.tsx`; that is the tool working as designed, not a gap to route around.

## Invocation

- [ ] **R008** Every tool is invoked through `pnpm` (`pnpm lint`, `pnpm knip`, ...), never through `npx` or a globally installed binary — `pnpm` resolves the exact pinned `devDependencies` version.
- [ ] **R009** A `package.json` script is the command itself, with no `echo`/brace-group/shell-plumbing wrapper and no `... | grep || echo "clean"` pipeline — a wrapper that swallows a tool's real exit code turns a failing gate into a silent pass.

## Commit convention

- [ ] **R010** A commit message is a single imperative subject line — no body, no trailers, no `Generated with` line. This is the standing rule from phase 03 onward. Commits made through the end of phase 02 carry exactly one trailer, `Co-Authored-By: <name> <noreply@anthropic.com>`, as project history from that period — that trailer is not part of the ongoing convention and is not added to any commit from phase 03 onward.
- [ ] **R011** One reviewable unit of work per commit.

## PR titles

- [ ] **R012** A pull request title follows Conventional Commits (`feat: ...`, `fix: ...`, `docs: ...`) — `main` squash-merges with the PR title as the resulting commit message (`PR_TITLE`), so the title is the commit history that persists, and R010's subject-line discipline applies to it directly.
