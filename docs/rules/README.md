...
| `general/design-mirror.md`          | The `/design-login` precondition, the DesignSync source-pull and mirror-push rituals (both main-session-only), `.design-sync/config.json`'s two pinned project identities, C2PA stripping, and the mirror's script-stripped/zero-network self-containment checks. |
